...
let convert_to_rgb (color : color) : (int * int * int) = 
  match validated_rgb color with 
  | RGB (x, y, z) -> (x, y, z)
...
let new_child (name : string) (clr : color) (bday : date) : family = 
  Single { name = name; 
           favorite = validated_rgb clr; 
           birthdate = validated_date bday } ;;
...
let marry (fam : family) (p : person) : family = 
  match fam with
  | Single p' -> Family (p', p, [])
  | Family (sp1, sp2, kids) -> raise (Family_Trouble "Already married") ;;
